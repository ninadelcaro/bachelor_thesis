...
keep_cols_hi <- c("subj", "expt", "session", "item", "lang", "roi", "FFD", "word_lex", "word_len")
...
colnames(df_hi) <- c("participant", "expt", "session", "sent_id_and_idx", "lang", "word_idx", "dur","word", "word_len")

keep_cols_hi2 <- c("participant", "sent_id_and_idx", "lang", "word_idx","dur","word", "word_len")
...
keep_cols_en <- c("uniform_id", "trialid", "sentnum", "ianum", "firstfix.dur", "ia")
df_en <- df_en[keep_cols_en]
# uniform_id == participant, firstfix.dur == dur, ia == word, trialid + sentnum == sent_id_and_idx, lang == en,
...
keep_cols_en2 <- c("participant", "trialid", "sent_id_and_idx", "lang", "word_idx", "firstfix.dur", "ia", "word_len", "ianum")
df_en_final <- df_en[keep_cols_en2]
colnames(df_en_final) <- c("participant", "text_id", "sent_id_and_idx", "lang", "word_idx", "dur", "word", "word_len", "total_word_idx")
